	...
	<div id="wrap" align="center">
		...
		<c:if test="${paging.numberOfRecords > 0}">
			...
			<tr>
				...
				<a href="/board/fsReg"><b class="underline-button" >게시글 등록</b></td></a>
			</tr>
			...
			<tr>
				<td colspan="5" style="text-align: center">
					<c:if test="${paging.startPageNo > paging.sizeOfPage}">
						<c class="page-button" href="<c:url value="/board/food.do?&p=${paging.prevPageNo}"/>">이전</c>
					</c:if>
					<c:forEach var="pno" begin="${paging.startPageNo}" end="${paging.endPageNo}">
						<c class="page-button" href="<c:url value="/board/food.do?&p=${pno}" />">[${pno}]</c>
					</c:forEach>
					<c:if test="${paging.endPageNo < paging.finalPageNo}">
						<c class="page-button" href="<c:url value="/board/food.do?&p=${paging.nextPageNo}"/>">다음</c>
					</c:if>
				</td>
			</tr>
		</c:otherwise>
